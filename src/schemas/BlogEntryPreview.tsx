...
function ProductGroupPreview({
  ...
  const [products, setProducts] = useState<Entity<BlogEntry>[] | undefined>();
  ...
  useEffect(() => {
    if (references) {
      Promise.all(
        ...
        .then((results) => results.filter((r) => !!r) as Entity<BlogEntry>[])
  ...
  return (
    <Box>
      {products.map((p, index) => (
        <ProductPreview
          key={`products_${index}`}
          productValues={p.values as EntityValues<BlogEntry>}
        />
      ))}
    </Box>
  );
...
export function ProductPreview({
  ...
  productValues: EntityValues<BlogEntry>;
}) {
